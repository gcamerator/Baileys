...
/*
  // 🟢 sharp (الأفضل)
  if (lib?.sharp?.default && typeof lib.sharp.default === 'function') {
    img = await lib.sharp
      .default(buffer)
      .resize(w, h)
      .jpeg({ quality: 50 })
  ...
  // 🟡 jimp (fallback)
    ...
    const jimpImg = await lib.jimp.read(buffer)
    ...
    jimpImg
      .crop(0, 0, min, min)
      .resize(w, h, lib.jimp.RESIZE_BILINEAR)

    img = await jimpImg.getBufferAsync(lib.jimp.MIME_JPEG)
  }
